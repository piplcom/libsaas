...
class HubspotTestCase(unittest.TestCase):
    def setUp(self):
        self.executor = test_executor.use()
        self.executor.set_response(b"{}", 200, {})

        self.service = hubspot.HubSpot("my-api-token")
    ...
    def expect(self, method=None, uri=None, params=None):
        ...
        if method in http.URLENCODE_METHODS:
            params.update({"hapikey": "my-api-token"})
        else:
            uri += "?hapikey=my-api-token"
            params = self.serialize(params)

        if uri:
            self.assertEqual(self.executor.request.uri, "https://api.hubapi.com" + uri)
    ...
    def test_companies(self):
        ...
        self.expect("GET", "/companies/v2/companies/paged")

        self.service.companies().get(500)
        self.expect("GET", "/companies/v2/companies/paged", {"limit": 500})

        self.service.companies().get(None, 1000)
        self.expect("GET", "/companies/v2/companies/paged", {"offset": 1000})

        self.service.companies().get(500, 1000)
        self.expect(
            "GET", "/companies/v2/companies/paged", {"limit": 500, "offset": 1000}
        )
    ...
    def test_company(self):
        ...
        self.service.company(1).get()
        self.expect("GET", "/companies/v2/companies/1")
        self.service.company(1).update(o)
        self.expect("PUT", "/companies/v2/companies/1", o)
        self.service.company(1).contacts()
        self.expect("GET", "/companies/v2/companies/1/contacts")

    def test_company_domain(self):
        a = {"subject": "test"}
        o = {"title": "My cool Brown"}

        self.service.companies_domain("pipl.com").get()
        expected_url = "/companies/v2/domains/pipl.com/companies"
        self.expect("POST", expected_url)

        self.service.companies_domain("pipl.com").get(500)
        self.expect("POST", expected_url, {"limit": 500})

        self.service.companies_domain("pipl.com").get(None, 1000)
        self.expect("POST", expected_url, {"offset": 1000})

        self.service.companies_domain("pipl.com").get(500, 1000)
        self.expect("POST", expected_url, {"offset": 1000, "limit": 500})
    ...
    #     self.expect("POST", "/authorizations", {"email": "u@d.com", "password": "boo"})
    #
    ...
    #     self.expect("DELETE", "/deals/1/products", {"product_attachment_id": 2})
    ...
    #     o = {"title": "My cool Brown"}
    #
    #     self.service.companies().get(start=100, limit=50)
    #     self.expect("GET", "/companies/v2/companies", paging)
    #     self.service.companies().create(o)
    #     self.expect("POST", "/companies/v2/companies", o)
    #     self.service.companies().find("brown")
    #     self.expect("GET", "/companies/v2/companies/find", {"term": "brown"})
    #     self.service.companies().delete(ids="1,2,3")
    #     self.expect("DELETE", "/companies/v2/companies", {"ids": "1,2,3"})
    #
    #     self.service.company(1).get()
    #     self.expect("GET", "/companies/v2/companies/1")
    #     self.service.company(1).merge(4)
    #     self.expect("POST", "/companies/v2/companies/1/merge", {"merge_with_id": 4})
    #     self.service.company(1).update(o)
    #     self.expect("PUT", "/companies/v2/companies/1", o)
    #     self.service.company(1).followers()
    #     self.expect("GET", "/companies/v2/companies/1/followers")
    #     self.service.company(1).activities(start=100, limit=50)
    #     self.expect("GET", "/companies/v2/companies/1/activities", paging)
    #     self.service.company(1).updates(start=100, limit=50)
    #     self.expect("GET", "/companies/v2/companies/1/updates", paging)
    #     self.service.company(1).files(start=100, limit=50)
    #     self.expect("GET", "/companies/v2/companies/1/files", paging)
    #     self.service.company(1).persons(start=100, limit=50)
    #     self.expect("GET", "/companies/v2/companies/1/persons", paging)
    #     self.service.company(1).deals(start=100, limit=50)
    #     self.expect("GET", "/companies/v2/companies/1/deals", paging)
    #
    ...
    #     self.service.goal(1).results(period_start="2013-01-01", period_end="2013-02-02")
    #     self.expect(
    #         "GET",
    #         "/goals/1/results",
    #         {"period_start": "2013-01-01", "period_end": "2013-02-02"},
    #     )
